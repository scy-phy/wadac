...
#Data from normal test db:
# test_idle_netCam <- idle_data[-train_idle_index,] %>% filter(t_label=="nestCam_idle")
# test_stream_netCam <- streaming_data[-train_stream_index,] %>% filter(t_label=="nestCam_staticStreaming"|t_label=="nestCam_liveStreaming")


# filenames2 <- c(filenames,list.files("/home/itrust/Desktop/IDS/sweta_nestcam/",pattern= "*.db"))
# 
...
conn <- dbConnect(driver,dbname=paste0("/home/itrust/Desktop/IDS/sweta_tplinkcam/",dbname))
...
attack_features <- attack_features[-which(attack_features$t_label=="tplinkcam_normalactions"),]
attack_tpcam <- list_files3[[i]][[2]]
attack_tpcam <- attack_tpcam[which(colnames(attack_tpcam)%in%colnames(packet_features))]
# attack_nestcam <- list_files2[[i]][[2]]
# attack_nestcam <- attack_nestcam[which(colnames(attack_nestcam)%in%colnames(packet_features))]
attack_features <- rbind(attack_features,attack_tpcam)
...
sensitivity_tplink_ddos = ifelse(!any(model_ddos$anomaly==0),0,sensitivity(check_acc))
specificity_tplink_ddos = ifelse(!any(model_ddos$anomaly==0),0,specificity(check_acc))
# sensitivity_tplink_ddos = sensitivity(check_acc)
# specificity_tplink_ddos = specificity(check_acc)

print(paste0("specificity of tplink ddos set = " , specificity_tplink_ddos))
print(paste0("sensitivity of tplink ddos set = " , sensitivity_tplink_ddos))

matrix_name <- substring(filenames[i], regexpr("bnl[0-9]{2}", filenames[i]),regexpr(".db", filenames[i])-1)
write.csv(check2,paste0("Results/confusion_matrix/tplink_ddos/",matrix_name,".csv"))


if(!is.na(check[2])){
  accuracy_ddos_tp = (check2[3]+check2[2])/(check2[1]+check2[2]+check2[3]+check2[4])
}
if(is.na(check[2])){
  accuracy_ddos_tp = check[1]/check[1]
}

fpr_ddos_tp <- check2[1]/(check[1]+check[2])
print(paste0("Accuracy of DDoS = ",accuracy_ddos_tp*100))



prediction_obj <- prediction(model_ddos$anomaly,model_ddos$actual_anomaly)
plot_precall(prediction_obj = prediction_obj,plot_name = paste0("/tplink_ddos/",substring(filenames[i], regexpr("bnl[0-9]{2}", filenames[i]),regexpr(".db", filenames[i])-1)))
...
ggplot(model_ddos, aes(x = as.numeric(rowname), y = Reconstruction.MSE, color = as.factor(anomaly))) +
  ...
  labs(x = "instance number",
       color = "Class")
ggsave(filename = paste0("Results/Anomaly_Plots/",bin_blk,"_tplink_ddos.png"))
