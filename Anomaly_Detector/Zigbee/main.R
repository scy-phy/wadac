...
filenames <- c()
filenames <- c(filenames,list.files("../../zigbee_dataset",pattern= "*.db"))
...
for(i in 1:length(filenames)){
  ...
  bin_blk <- substring(filenames[i], regexpr("bk[0-9]{2}", filenames[i]),regexpr(".db", filenames[i])-1)
