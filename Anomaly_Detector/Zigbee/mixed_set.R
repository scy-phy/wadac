library(ROCR)
model_zigbee_mixed = as.data.frame(h2o.anomaly(model_zigbee, mixed.hex, per_feature=F))
model_zigbee_mixed$bla_stream <- sort(model_zigbee_mixed$Reconstruction.MSE)
# model_zigbee_mixed$label_no <- randoming_data$t_label
ggplot(data = model_zigbee_mixed,aes(y =bla_stream,x=c(1:nrow(model_zigbee_mixed)))) + geom_point() +ylim(0,0.05) # eRROR = 0.005

model_zigbee_mixed$t_label <- labels[[4]]
model_zigbee_mixed <- model_zigbee_mixed %>% mutate(anomaly = ifelse((Reconstruction.MSE >=zigbee_limit_upper | Reconstruction.MSE <= zigbee_limit_low),1,0 ),
                                                    actual_anomaly = ifelse(!grepl("normal",t_label),1,0)
                                                    )

check4 <- table(model_zigbee_mixed$anomaly,model_zigbee_mixed$actual_anomaly)
print(check4)
...
check5 <- table(model_zigbee_mixed$t_label,model_zigbee_mixed$anomaly)
print(check5)
...
prediction_obj <- prediction(model_zigbee_mixed$anomaly,model_zigbee_mixed$actual_anomaly)
plot_precall(prediction_obj = prediction_obj,plot_name = paste0("/mixed_set/",bin_blk))


mean_mse <- model_zigbee_mixed %>%
  group_by(anomaly) %>%
  summarise(mean = mean(Reconstruction.MSE))

model_zigbee_mixed <- model_zigbee_mixed %>%  tibble::rownames_to_column()
ggplot(model_zigbee_mixed, aes(x = as.numeric(rowname), y = Reconstruction.MSE, color = as.factor(anomaly))) +
...
for(k in 1:length(j)){
  model_zigbee3 <- model_zigbee_mixed %>% mutate(anomaly = ifelse((Reconstruction.MSE >=zigbee_limit_upper_seq[k] | Reconstruction.MSE <= zigbee_limit_low ),1,0 ))
  
  table(model_zigbee3$anomaly)
  model_zigbee3_actualanom <- model_zigbee_mixed$actual_anomaly
  
  check3 <- table(model_zigbee3$anomaly,model_zigbee3_actualanom)
  print(check3)
