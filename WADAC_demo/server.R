server <- function(input, output) {
  ...
  future(system("python3 live_capture_n_extract_features_anomaly_detect.py"))
  ...
  apply_ad = reactive({
    ...
    cols_keep = c(cols_keep,"Timestamp")
    all_data <- all_data %>% mutate(Timestamp = as.POSIXct(f_time_fstframe,origin="1970-01-01",tz = Sys.timezone())) %>% select(-f_time_fstframe) %>% select(cols_keep)
    x = ymd_hms("2018-09-19 16:00:00", tz = Sys.timezone())
    mins = c()
    #print(nrow(all_data))
    for( i in 1:nrow(all_data)){mins = c(mins,i*60)}
    #print(mins)
    all_data = all_data %>% mutate(Timestamp = mins+x)
